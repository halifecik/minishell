...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/07/03 16:21:18 by hademirc          #+#    #+#             */
/*   Updated: 2025/07/03 16:21:19 by hademirc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	check_char(char **current_char, char **start)
{
	...
	if (**current_char == '\'')
		...
		check_parentheses(current_char, start);
	else if (**current_char == '|' && *((*current_char) + 1) == '|')
		check_operator_or(current_char, start);
	else if (**current_char == '&' && *((*current_char) + 1) == '&')
		check_operator_and(current_char, start);
	else if (**current_char == '|')
		check_pipe(current_char, start);
	else if (**current_char == '>' || **current_char == '<')
		check_redirections(current_char, start);
	else if (**current_char == '*')
		check_wildcard(current_char, start);
